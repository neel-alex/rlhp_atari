...
from pynvml import * # pip install nvidia-ml-py3
...
def find_free_gpu():
    ...
    nvmlShutdown()
    sort_by_mem = list(zip(range(deviceCount), free_mem)).sort(key=lambda y: y[1])
    return sort_by_mem[-1][0]
...
def make_env(env_id, rank, log_dir, seed=0, env_wrappers=None):
    def _init():
        env = gym.make(env_id)
        env.seed((seed + rank))
...
# TODO: make suitable for Atari...
def make_train_env(env_id, save_dir, base_seed=0, num_threads=1,
                   normalize_obs=True, normalize_reward=True,
                   env_wrappers=None, vec_wrappers=None, **kwargs):
    env = [make_env(env_id, i, save_dir, base_seed, env_wrappers)
           for i in range(num_threads)]
    env = vec_env.SubprocVecEnv(env)
    if vec_wrappers is not None:
        for wrapper in vec_wrappers:
            env = wrapper(env)
#            print(env.observation_space.sample().shape)
    if normalize_reward or normalize_obs:
        assert(all(key in kwargs for key in ['gamma']))
...
def make_eval_env(env_id, normalize_obs=True, env_wrappers=None, vec_wrappers=None):
    env = [make_env(env_id, i, None, np.random.randint(100), env_wrappers)
           for i in range(1)]
    #env = vec_env.SubprocVecEnv(env)
    ...
    env = vec_env.VecNormalize(
           env, training=False, norm_obs=normalize_obs, norm_reward=False)
...
def get_net_arch(config):
    ...
    if config.rl_algo in ['ppo', 'trpo']:
        ...
        separate_layers = dict(pi=config.policy_layers,    # Policy Layers
                               qf=config.reward_vf_layers) # Value Function Layers
        return separate_layers
