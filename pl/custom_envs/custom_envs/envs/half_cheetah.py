...
class HalfCheetahWithPos(HalfCheetahEnv):
    ...
    def actual_env_reward(self, xposbefore, xposafter, action):
        reward_ctrl = -0.1 * np.square(action).sum()
        reward_run = (xposafter - xposbefore) / self.dt
        reward = reward_ctrl + reward_run
    ...
    def modified_env_reward(self, xposbefore, xposafter, action):
        reward_ctrl = -0.1 * np.square(action).sum()
    ...
    def step(self, action):
        ...
        xposafter = self.sim.data.qpos[0]
        ob = self._get_obs()
        reward, info = self.actual_env_reward(xposbefore,
                                              xposafter,
                                              action)
        done = False
