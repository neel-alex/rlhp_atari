import warnings
from typing import Generator, Optional, Tuple, Union, Callable
...
class ReplayBufferWithRM(ReplayBuffer):
    def __init__(
        self,
        ...
        super(ReplayBufferWithRM, self).__init__(buffer_size, observation_space, action_space, device, n_envs, optimize_memory_usage)
        self.reward_model = reward_model
        self.reward_relabeling = reward_relabeling
        self.ret_rms = RunningMeanStd(shape=())
        self.last_rm_call = 0

    def _get_rewards(self, obs, acs, next_obs, dones):
        rewards = self.reward_model(obs.copy(), acs.copy(),
                        next_obs.copy(), dones.copy()).cpu().detach().numpy()[...,None]
        self.ret_rms.update(rewards)
        # Weirdly using reward normalization makes SAC+RM much worse
        #rewards = rewards / np.sqrt(self.ret_rms.var + self.ret_rms.count)
        return rewards

    def _get_next_obs(self, inds):
        if self.optimize_memory_usage:
            next_obs = self.observations[(inds + 1) % self.buffer_size, 0, :]
        else:
            next_obs = self.next_observations[inds, 0, :]
        return next_obs
...
class CustomRolloutBuffer(RolloutBuffer):
    ...
    def reset(self) -> None:
        ...
        self.next_orig_observations = np.zeros((self.buffer_size, self.n_envs) + self.obs_shape, dtype=np.float32)
        super(CustomRolloutBuffer, self).reset()

    def add(
            self, obs: np.ndarray, orig_obs:np.ndarray, next_obs: np.ndarray, next_orig_obs: np.ndarray,
        ...
        self.next_observations[self.pos] = np.array(next_obs).copy()
        self.next_orig_observations[self.pos] = np.array(next_orig_obs).copy()
